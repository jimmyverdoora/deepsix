function v = alphabeta(node, depth, alpha, beta, player, net)
    ...
    if depth == 0
        if size(possible(node,player),1) == 1
            ...
            v = player*net(player*reshape(node,[125,1]));
        end
    ...
    elseif player == 1
        ...
        if size(moves,1) == 1
            ...
            for i = 1:size(moves,1)
                v = max(v, alphabeta(moves{i}, depth-1, alpha, beta, -1, net));
                alpha = max(alpha, v);
                if beta <= alpha
                   break % (* beta cut-off *)
                end
        ...
        if size(moves,1) == 1
            ...
            for i = 1:size(moves,1)
                v = min(v, alphabeta(moves{i}, depth-1, alpha, beta, 1, net));
            	beta = min(beta, v);
                if beta <= alpha
                     break % (* alpha cut-off *)
                end
